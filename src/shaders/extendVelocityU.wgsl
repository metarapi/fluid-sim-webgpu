...
@group(0) @binding(1) var<storage, read_write> uGridMask: array<u32>;
@group(0) @binding(2) var<uniform> params: SimParams;

// Kernel for extending U velocities
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (uGridMask[pid] != 0u) { return; }
    
    // Get grid coordinates
    let i = pid % (params.size_x + 1);
    let j = pid / (params.size_x + 1);
    ...
    // 3. Check if any neighbor has been written to (has mask != 0)
    for (var k = 0u; k < 8u; k++) {
        let ni = i32(i) + offsets[k].x;
        let nj = i32(j) + offsets[k].y;
        
        // Check bounds
        if (ni < 0 || ni >= i32(params.size_x + 1) || 
            nj < 0 || nj >= i32(params.size_y)) {
            continue;
        }
        
        // Get neighbor index
        let nIdx = u32(ni) + u32(nj) * (params.size_x + 1);
        
        // Check if this neighbor has velocity data
        if (uGridMask[nIdx] != 0u) {
    ...
    for (var k = 0u; k < 8u; k++) {
        ...
        accumVel += uGrid[nIdx] * weight;
        validCount += 1u;
    }
    
    // 6. Write the velocity if we have valid neighbors
    if (validCount > 0u) {
        uGrid[pid] = accumVel / f32(validCount);
        // Mark this node as having valid data for next pass
        uGridMask[pid] = 2u; // 2 indicates extended velocity (vs 1 for original particle contribution)
    }
}