...
@group(0) @binding(1) var<storage, read_write> numCellParticles: array<atomic<u32>>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  // let cellIndex = (cellX + 1) + cellY * params.size_x;
  let cellIndex = cellX + cellY * params.size_x;
  
  atomicAdd(&numCellParticles[cellIndex], 1u);
}