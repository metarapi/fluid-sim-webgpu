...
@group(0) @binding(4) var<storage, read_write> vGrid: array<f32>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Origin cell
    let cellX = pid % (params.size_x);
    let cellY = pid / (params.size_x);

    // Node position in world space
    let nodeX = (f32(cellX) + 0.5) * params.grid_to_world_x;
    let nodeY = f32(cellY) * params.grid_to_world_y;

    // Loop over neighboring cells in the regular grid
    for (var xx = i32(cellX)-1; xx <= i32(cellX)+1; xx += 1) {
        for (var yy = i32(cellY)-1; yy <= i32(cellY); yy += 1) {
            if (xx >= 0 && u32(xx) < params.size_x && yy >= 0 && u32(yy) < params.size_y) {
                ...
                for (var pIdx = start; pIdx < end; pIdx += 1) {
                    ...
                    if (abs(particle.pos.x - nodeX) < params.grid_to_world_x && abs(particle.pos.y - nodeY) < params.grid_to_world_y) {
                        ...
                        accumVel += particleVel[p].vel.y * weight;
                        totalWeight += weight;
                    }
                }
            }
    ...
    if (totalWeight > 0.0) {
        vGrid[pid] = accumVel / totalWeight;
    } else {
        vGrid[pid] = 0.0;
    }
}