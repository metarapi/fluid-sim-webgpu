...
@group(0) @binding(0) var<storage, read_write> residual: array<f32>; // r
@group(0) @binding(1) var<storage, read> temp: array<f32>; // q = A·p
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  residual[idx] = residual[idx] - alpha * temp[idx];
}