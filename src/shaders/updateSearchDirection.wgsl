...
@group(0) @binding(0) var<storage, read_write> searchDirection: array<f32>; // p
@group(0) @binding(1) var<storage, read> aux: array<f32>; // z
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  if (cellType[idx] != LIQUID) {
    searchDirection[idx] = 0.0;
    return;
  }
  
  // Read beta from params buffer
  let beta = pcgParams.beta;
  
  // Update search direction: p = z + beta * p
  searchDirection[idx] = aux[idx] + beta * searchDirection[idx];
}